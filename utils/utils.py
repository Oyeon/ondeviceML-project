...
def build_targets(p, targets, model):
    ...
    at = torch.arange(na, device=targets.device).view(na, 1).repeat(1, nt)  # anchor tensor on same device

    # Define the style here (choose 'rect4' as default)
    style = 'rect4'  # Options can be 'rect2' or 'rect4'

    for i in range(det.nl):
        anchors = det.anchors[i].to(targets.device)  # ensure anchors are on the same device as targets
        gain[2:] = torch.tensor(p[i].shape)[[3, 2, 3, 2]].to(targets.device)  # ensure gain tensor is on the right device
        ...
        if nt:
            ...
            j = j.to(targets.device)  # ensure j is on the correct device
            a, t = at[j], t.repeat(na, 1, 1)[j]  # filter

            # Overlaps logic based on the style
            gxy = t[:, 2:4]  # grid xy
...
def output_to_target(output, width, height):
    ...
    """
    if isinstance(output, torch.Tensor):
        output = output.cpu().numpy()  # Move to CPU before converting to NumPy

    targets = []
    for i, o in enumerate(output):
        if o is not None:
            for pred in o:
                box = pred[:4].cpu().numpy()  # Ensure it's on CPU
                w = (box[2] - box[0]) / width
                h = (box[3] - box[1]) / height
                x = box[0] / width + w / 2
                y = box[1] / height + h / 2
                conf = pred[4].cpu().numpy()  # Ensure it's on CPU
                cls = int(pred[5].cpu())  # Ensure it's on CPU
