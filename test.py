...
def test(data,
    ...
    # plot_images(img.cpu(), output_to_target(output, width, height), paths, f, names)  # ensure img is on CPU for predictions
    
        if batch_i < 1:
            f = 'test_batch%g_gt.jpg' % batch_i  # filename
            plot_images(img.cpu(), targets.cpu(), paths, f, names)  # ensure img and targets are on the CPU for ground truth
